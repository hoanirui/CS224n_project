...
class LSTM_attention_Config(object):
    ...
    def __init__(self, debug=False):
        ...
        self.vocab_size = 9828 # (no of words in the vocabulary)
...
class LSTM_attention_Model(object):
    ...
    def load_utilities_data(self):
        ...
        # load the vocabulary:
        self.vocabulary = cPickle.load(open("coco/data/vocabulary", "rb"))

        # load data to map from caption id to img id:
        self.caption_id_2_img_id =\
                    cPickle.load(open("coco/data/caption_id_2_img_id", "rb"))

        # load data to map from caption id to caption:
        if self.debug:
            self.train_caption_id_2_caption =\
                    cPickle.load(open("coco/data/val_caption_id_2_caption", "rb"))
        else:
            self.train_caption_id_2_caption =\
                    cPickle.load(open("coco/data/train_caption_id_2_caption", "rb"))

        # load data to map from img id to feature array:
        self.img_id_2_feature_array =\
                cPickle.load(open("coco/data/img_id_2_feature_array", "rb"))

        # load data needed to create batches:
        if self.debug:
            self.caption_length_2_caption_ids =\
                cPickle.load(open("coco/data/val_caption_length_2_caption_ids", "rb"))
            self.caption_length_2_no_of_captions =\
                cPickle.load(open("coco/data/val_caption_length_2_no_of_captions", "rb"))
        else:
            self.caption_length_2_caption_ids =\
                cPickle.load(open("coco/data/train_caption_length_2_caption_ids", "rb"))
            self.caption_length_2_no_of_captions =\
                cPickle.load(open("coco/data/train_caption_length_2_no_of_captions", "rb"))
...
def main():
    ...
    #loss_per_epoch = []
    loss_per_epoch = cPickle.load(open("models/LSTMs_attention/model_keep=0.75_batch=256_hidden_dim=400_embed_dim=300_layers=1_hidden_dim_att=500/losses/loss_per_epoch", "rb"))
    ...
    eval_metrics_per_epoch = cPickle.load(open("models/LSTMs_attention/model_keep=0.75_batch=256_hidden_dim=400_embed_dim=300_layers=1_hidden_dim_att=500/eval_results/metrics_per_epoch", "rb"))
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, "models/LSTMs_attention/model_keep=0.75_batch=256_hidden_dim=400_embed_dim=300_layers=1_hidden_dim_att=500/weights/model-60")

        for epoch in range(61, config.max_no_of_epochs):
            print("###########################")
            print("######## NEW EPOCH ########")
            print("###########################")
            print("LSTM 500 epoch: %d/%d" % (epoch, config.max_no_of_epochs-1))
            log("###########################")
            log("######## NEW EPOCH ########")
            log("###########################")
            log("LSTM 500 epoch: %d/%d" % (epoch, config.max_no_of_epochs-1))
            ...
            if eval_result_dict["CIDEr"] > 0.885 or epoch == config.max_no_of_epochs - 1:
                # save the model weights to disk:
                saver.save(sess, "%s/weights/model" % model.config.model_dir,
                            global_step=epoch)

            print("LSTM epoch loss: %f | BLEU4: %f  |  CIDEr: %f" % (epoch_loss,
                        eval_result_dict["Bleu_4"], eval_result_dict["CIDEr"]))
            log("LSTM epoch loss: %f | BLEU4: %f  |  CIDEr: %f" % (epoch_loss,
                        eval_result_dict["Bleu_4"], eval_result_dict["CIDEr"]))
