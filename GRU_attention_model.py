...
def main():
    ...
    loss_per_epoch = cPickle.load(open("models/GRUs_attention/model_keep=0.75_batch=256_hidden_dim=400_embed_dim=300_layers=1_hidden_dim_att=500/losses/loss_per_epoch", "rb"))
    eval_metrics_per_epoch = cPickle.load(open("models/GRUs_attention/model_keep=0.75_batch=256_hidden_dim=400_embed_dim=300_layers=1_hidden_dim_att=500/eval_results/metrics_per_epoch", "rb"))
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, "models/GRUs_attention/model_keep=0.75_batch=256_hidden_dim=400_embed_dim=300_layers=1_hidden_dim_att=500/weights/model-65")

        for epoch in range(66, config.max_no_of_epochs):
            print("###########################")
            print("######## NEW EPOCH ########")
            print("###########################")
            print("GRU 500 epoch: %d/%d" % (epoch, config.max_no_of_epochs-1))
            log("###########################")
            log("######## NEW EPOCH ########")
            log("###########################")
            log("GRU 500 epoch: %d/%d" % (epoch, config.max_no_of_epochs-1))
            ...
            if eval_result_dict["CIDEr"] > 0.89 or epoch == config.max_no_of_epochs - 1:
                # save the model weights to disk:
                saver.save(sess, "%s/weights/model" % model.config.model_dir,
                            global_step=epoch)

            print(" GRU epoch loss: %f | BLEU4: %f  |  CIDEr: %f" % (epoch_loss,
                        eval_result_dict["Bleu_4"], eval_result_dict["CIDEr"]))
            log("GRU epoch loss: %f | BLEU4: %f  |  CIDEr: %f" % (epoch_loss,
                        eval_result_dict["Bleu_4"], eval_result_dict["CIDEr"]))
