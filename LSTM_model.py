...
class LSTM_Model(object):
    ...
    def load_utilities_data(self):
        ...
        if self.debug:
            self.train_img_id_2_feature_vector =\
                    cPickle.load(open("coco/data/val_img_id_2_feature_vector","rb"))
        else:
            self.train_img_id_2_feature_vector =\
                    cPickle.load(open("coco/data/train_img_id_2_feature_vector","rb"))
    ...
    def add_input(self):
        ...
        with tf.variable_scope("img_transform", reuse=tf.AUTO_REUSE):
            ...
            #numpy_params = {"W_img" : W_img, "b_img" : b_img}
            #with open("coco/data/img_features_attention/transform_params/numpy_params", "wb") as f:
            #    cPickle.dump(numpy_params, f)
            #log("Numpy_params")
            #log("*********************************")
        ...
        with tf.variable_scope("captions_embed", reuse=tf.AUTO_REUSE):
    ...
    def generate_captions_on_val(self, session, epoch, vocabulary, val_set_size=5000):
        ...
        val_img_id_feature_vector_list = list(val_img_id_2_feature_vector.items())
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(config.max_no_of_epochs):
            ...
            # change the value from 0.92 to 0.86 to get weight values.
            if eval_result_dict["CIDEr"] > 0.86:
